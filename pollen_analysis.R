...
# Walentowitz et al.
# last edited on 21. February 2023
...
### data preparation and single island analysis
...
for(i in 1:nrow(env)){
  if(!is.na(env$pollen_assign[i]) &&
    ...
    pollen_df$sum.inv.lower <- round(rowSums(pollen_lower[which(colnames(pollen_lower) %in% ali)]), digits = 2)
    pollen_df$sum.inv.upper <- round(rowSums(pollen_upper[which(colnames(pollen_upper) %in% ali)]), digits = 2)
    pollen_df$sum.endem     <- round(rowSums(pollen_raw[which(colnames(pollen_raw) %in% endemic)]), digits = 2)
    pollen_df$sum.crops     <- round(rowSums(pollen_raw[which(colnames(pollen_raw) %in% crop)]), digits = 2)
    ...
    # Select window size so that ~ 25 averages are being made.
    ...
    # vertical black line indicating human arrival
...
# remove Foa and Ha'afeva from multi-island analysis as the pollen data had a very low temporal distribution
...
mean(pollen_all$sum.inv.upper[which(pollen_all$time < 280)])# 19%
...
### multi-island plots
# plot pollen abundance 5000 years
...
pollen_all_endem <- pollen_all[which(pollen_all$sum.endem > 0 & !is.na(pollen_all$sum.endem)),]
...
length(unique(pollen_tax_filtered$tax_filtered)) # 665
